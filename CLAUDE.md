...
| sku | id, product_id, spec_name, price, member_price, duration, stock, sort, create_time, update_time | 规格与价格明细（库存、SERVICE 时长 duration 也在 SKU 上，单位分钟） | HomePage 详情/规格弹窗；CartPage/CheckoutPage 按 SKU 算价；下单时按 SKU 落单 | ProductPage 规格管理（规格名、原价、会员价、库存、排序） |
...
| orders | id, order_no, user_id, customer_phone_snapshot, customer_name, member_id, member_level_snapshot, goods_amount, service_amount, delivery_fee, total_amount, need_delivery, delivery_address, delivery_lat, delivery_lng, delivery_distance, processed, cancelled, remark, create_time, update_time | 订单主表（金额、配送、处理状态、快照） | CheckoutPage 创建订单；OrderListPage/OrderDetailPage 查询订单；OrderSuccessPage 展示下单结果 | OrderPage 列表筛选、详情查看、标记已处理/未处理 |
...
| system_config | id, shop_lat, shop_lng, delivery_radius_km, delivery_min_amount, delivery_fee_type, fixed_delivery_fee, order_time_enabled, order_start_time, order_end_time, qywx_webhook_url_enc, has_qywx_webhook, payment_qr_url, updated_by, create_time, update_time | 全局系统配置（配送规则、预约时段、通知、收款码） | CheckoutPage/cart calculate、下单流程：配送可达、起送价、运费规则（经后端） | SystemConfigPage 配置读写、Webhook 测试。注：delivery_fee_type 现仅支持 FREE/TIERED（FIXED 已下线，前端无入口，后端兼容历史数据） |
