...
> ✅ **状态：已完成，2026-07-16 随 `b99d15c` 上线生产**。首页每天首次进入弹一张中间悬挂广告图，右上角圆形按钮可关；老板在 Admin 上传图 + 开关控制（生产 `ad_enabled=0` 默认关，Admin 配置图 + 打开开关后对 C 端生效）。
...
## 单 SKU 商品弹窗选购（2026-07-16）

> ✅ **状态：已完成并上线**（`b99d15c`）。单 SKU 商品原先只能点 `+` 直接加购、看不到完整商品描述，改为复用多 SKU 的「选规格」弹窗。

### 改动
- `frontend/h5/src/components/home/ProductCard.vue`：单 SKU 分支由 `+/-` 数量控件改为「选购」按钮，`@click="$emit('selectSpec', product)"`，与多 SKU 走同一链路；按钮文案按 `hasSpec` 切换「选规格 / 选购」。移除 `qty-control` 模板与样式、`changeQty` emit、`cartQty` computed、`useCartStore` 引入。
- `frontend/h5/src/views/HomePage.vue`：移除 `@change-qty` 绑定与 `handleDirectQty`（及随之失效的 `useCartStore` / `cartStore`）。单 SKU 直接复用现有 `openSpecPicker`（拉详情 → 开 `SkuSelectorPopup`）。
- `frontend/h5/src/components/product/SkuSelectorPopup.vue`：`.sku-desc` 去掉 `-webkit-line-clamp:2`，改 `white-space:pre-wrap` 完整展示——单/多 SKU 弹窗均能看完整商品描述（本次核心诉求）。
- 主服务「去预约」、多 SKU「选规格」分支不变；后端无改动（`GET /api/app/products/{id}` 已返回完整 `description`）。

### 边界
- 单 SKU 加购的购物车 key 从 `skuId:null` 变为真实 `skuId`；`CartDrawer` 加减基于 item 自身 `skuId`，自洽。持久化旧 `null` 记录不主动迁移（下单后自然消失）。

### 验证
`pnpm build`（含 `vue-tsc`）通过；Chrome DevTools 端到端：单 SKU「选购」→ 弹窗完整描述 + 加购（购物车 +1）、多 SKU「选规格」、主服务「去预约」回归均正常。

## 生产部署：同步 b99d15c（2026-07-16）

服务器 `106.53.178.130` 从 `cd1b890` 同步到 `b99d15c`，上线积压的「开屏广告」全栈 + 本次「单 SKU 弹窗」修复。

- **DB 迁移**：执行 `backend/sql/migration_ad_v1.sql`，`system_config` 加 `ad_enabled/ad_image_url/ad_link_type/ad_link_target` 4 列（`ad_enabled` 默认 0）。⚠️ 必须先于 backend 重建——新 `SystemConfigMapper.xml` 的 SELECT/INSERT/UPDATE 均含 `ad_*` 列，缺列则 backend 启动即 `Unknown column`。
- **容器重建**：`docker compose -f docker-compose.prod.yml --env-file .env.prod up -d --build --force-recreate backend frontend`（mysql 不动）。
- **验证**：三容器正常（backend healthy）、日志无 SQL 错误；`/api/app/system-config/public` 返回含 `"adEnabled":false`（backend 广告代码 + DB 列双生效）；H5/Admin/商品接口 HTTPS 200。
- **服务器侧验证技巧**：nginx 已强制 HTTPS，`curl http://localhost` 返回 301，改用 `curl -k --resolve 2zg.site:443:127.0.0.1 https://2zg.site/<path>`（绕 hairpin NAT + 匹配域名/证书）。
