...
| B | 后端配置外部化（profile/CORS/密钥/密码） | ⏳ 待执行（见第 2 节） |
| C | 部署产物落盘（Dockerfile/nginx/compose/.env） | ⏳ 待执行（见第 3 节） |
| D | 安全清理（.gitignore + 凭据移仓） | ⏳ 待执行（见第 4 节） |
| E | HTTPS + 域名 + Admin 隐秘路径 | ⏳ 待执行（见第 3、5 节） |
| F | 本地验证 | ⏳ 待执行（见第 6 节） |
| G | 服务器上线 | ⏳ 待执行（见第 7 节） |
...
| **域名 DNS 解析** | ⚠️ 待用户操作 | `2zg.site` + `www.2zg.site` → `106.53.178.130`（域名商后台） |
| **MySQL root 强密码** | ⚠️ 待用户生成 | 部署时生成随机值填 `.env.prod` |
| **Admin 强密码** | ⚠️ 待用户设定 | 部署时生成填 `ADMIN_INIT_PASSWORD` |

---

## 6. 本地验证 ⏳

- [ ] 后端 `mvn clean package -DskipTests` 确认 jar 产出 + 模块 B 改造无误
- [ ] 前端 admin 带 `VITE_BASE_URL=/petshop-admin-7x9k2/` 构建正确（已验证一次）
- [ ] （可选）Docker 镜像本地构建通过
...
### 步骤 9：验证
- [ ] 访问 `https://2zg.site/` → H5 首页
- [ ] 访问 `https://2zg.site/petshop-admin-7x9k2/` → Admin 登录页
- [ ] 访问 `http://2zg.site/` → 自动跳 HTTPS
- [ ] 访问 `https://www.2zg.site/` → 跳根域
- [ ] 测试 Admin 登录、图片上传
- [ ] **腾讯地图后台已配 `2zg.site` 域名白名单**（否则地图加载失败）
- [ ] `SMS_PROVIDER=aliyun` 后用真实手机号测验证码登录（默认 log 模式固定码 1234 会误导）

### 步骤 10：上线后必做
- [ ] 立即修改 admin 默认密码（登录后改）
- [ ] 确认 MySQL 容器未映射 3306 到宿主机（prod compose 已用 `expose` 仅内网，复查）
- [ ] 配数据备份 cron（**用 mysqldump --single-transaction，勿冷拷运行中的 InnoDB**）：
  ```bash
